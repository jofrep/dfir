...
2)A quick look at the script shows this is only concatenating strings and decompressing them to generate a code. This is executed in the last line.
...
9) We can now generate all the domains used in the past, and future (f23.svg_dga) with the following code:
```powershell
for($j = 1; $j -lt 365; $j++)
{
